...
libdebuff:SetScript("OnEvent", function()
  ...
  elseif ( event == "UNIT_AURA" and arg1 == "target" ) or event == "PLAYER_TARGET_CHANGED" then
    -- OPTIMIZATION: Skip UNIT_AURA processing for "Own Debuffs Only" mode
    -- We already have all info from AURA_CAST events, no scanning needed!
    if isEnhancedMode and pfUI_config and pfUI_config.unitframes then
      local debuffFilter = pfUI_config.unitframes.target and pfUI_config.unitframes.target.debufffilter
      if debuffFilter == "own" then
        -- Own debuffs only: Skip slot mapping entirely!
        -- AURA_CAST/AURA_FADE events give us everything we need
        return
      end
    end
...
function libdebuff:UnitDebuff(unit, id)
  ...
  -- OPTIMIZATION: "Own Debuffs Only" mode - skip slot scanning, iterate enhancedDebuffs directly
  if isEnhancedMode and pfUI_config and pfUI_config.unitframes then
    local debuffFilter = pfUI_config.unitframes.target and pfUI_config.unitframes.target.debufffilter
    if debuffFilter == "own" then
      ...
      if unitGUID and IsValidGUID(unitGUID) and enhancedDebuffs[unitGUID] then
        ...
        table.sort(ownDebuffs, function(a, b) return a.remaining > b.remaining end)
        
        -- Return the id-th own debuff
        if ownDebuffs[id] then
          local debuff = ownDebuffs[id]
          effect = debuff.spellName
          duration = debuff.data.duration
          timeleft = debuff.remaining
          caster = "player"
          
          -- Get texture from spell_cache (no slot scanning needed!)
          if pfUI_cache and pfUI_cache.spell_cache and debuff.spellID then
            texture = pfUI_cache.spell_cache[tostring(debuff.spellID)]
          end
  ...
  -- Normal path: Get info from slot
