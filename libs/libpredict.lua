...
function libpredict:GetHotDuration(unit, spell)
  ...
  -- Enhanced Mode: Use libdebuff if available
  if C and C.unitframes and C.unitframes.enhanced_tracking == "1" then
    local libdebuff = pfUI.api and pfUI.api.libdebuff
    
    if libdebuff and libdebuff.GetEnhancedDebuffs then
      ...
      if unitGUID and playerGUID then
        ...
        local spellName = spellMap[spell]
        if spellName then
          local enhancedData = libdebuff:GetEnhancedDebuffs(unitGUID)
          
          if enhancedData and enhancedData[spellName] then
            local bestStart, bestDuration, bestTimeLeft = nil, 0, 0
            
            -- Check if shared buff
            local isShared = libdebuff.IsSharedAura and libdebuff:IsSharedAura(spellName)
            
            if isShared then
              -- Shared: longest timer from ANY caster
              for casterGUID, data in pairs(enhancedData[spellName]) do
                if data.startTime and data.duration then
                  local endTime = data.startTime + data.duration
                  local remaining = endTime - now
                  
                  if remaining > bestTimeLeft then
                    bestStart = data.startTime
                    bestDuration = data.duration
                    bestTimeLeft = remaining
                  end
                end
              end
            else
              -- Own only
              local data = enhancedData[spellName][playerGUID]
            ...
            if bestStart and bestDuration and bestTimeLeft > 0 then
              return bestStart, bestDuration, bestTimeLeft
            end
          end
        end
      end
