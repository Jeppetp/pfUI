pfUI:RegisterModule("nameplates", "vanilla", function ()
  ...
  nameplates.OnUpdate = function(frame)
    ...
    -- PERF: Detect target by comparing nameplate name to target unit name
    local plateName = nameplate.original.name:GetText()
    local target = UnitExists("target") and plateName and plateName == UnitName("target") or nil
    -- PERF: Increase throttle for non-target plates (0.05s vs 0.02s for target)
    -- In combat, further increase non-target throttle to 0.1s for raid performance
    local inCombat = nameplates.combat.inCombat
    local throttle = target and 0.020 or (inCombat and 0.1 or 0.05)
    ...
    if nameplate.cachedAlpha ~= newAlpha then
      -- Store the alpha for use in color-setting code
      nameplate.inactiveAlpha = newAlpha

      if newAlpha < 0.05 then
        -- Very low alpha: hide completely
        nameplate:Hide()
      else
        nameplate:Show()
      end
